...
select ename, sal, comm from emp where not(comm=0 or comm is null) order by sal, comm;

-- 12. 이름의 세번째 문자가 R인 사원의 이름을 표시하시오.
select * from emp where ename like '__R%';

-- 13. 이름에 A와 E를 모두 포함하고 있는 사원의 이름을 표시하시오.
select * from emp where (ename like '%A%')and(ename like '%E%') OR (ename like '%E%')and(ename like '%A%');
