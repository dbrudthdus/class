...
select *
from emp
where job != 'ASALYST' and sal < all(select distinct sal from emp where job='ASALYST')
;

select distinct sal from emp where job='ASALYST';
...
-- emp => mgr(상사) => 사원의 mgr 컬럼에 KING 사원의  사원번호를 가지고 있으면 => KING은 상사이다.
...
select *
from emp
where deptno in(select distct deptno from emp where job='MANAGER')
;
