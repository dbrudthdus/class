...
-- 인라인뷰로 입사일을 정렬해주니까 로우넘 번호도 같이 정렬됨.
select rownum, ename, hiredate
from (select ename, hiredate from emp order by hiredate) -- 입사일 정렬
where rownum<=3 -- 정렬된 순서 1,2,3번 출력  
;
...
-- 3000 이상 받는 사원이 소속된 부서(10번, 20번)와 동일한 부서에서 근무하는 사원
...
select * from orders where custid=3; -- 3번 고객이 주문한 내역
select max(saleprice) from orders where custid=3; -- 3번 고객이 주문한 최고 금액 
